...
var   _                     = require('underscore')
    ...
    , VectorTileInputFactory = require('./VectorTileInput.js')
    , util                   = require('util')
;
...
module.exports = function(timeout, mml_store, map_store, mapnik_opts) {
    ...
    if ( mapnik_opts.vector_datasources ) {
      var vtInputFactory = new VectorTileInputFactory(mapnik_opts.vector_datasources);
      tilelive.protocols['windshaft:'] = vtInputFactory.getHandler();
    }
        ...
        Step(
            ...
            function loadMapnik(err, data){
                ...
                if ( doing_vector ) {
                  ...
                  for ( var i=0; i<cfg.layers.length; ++i ) {
                    var lyr = cfg.layers[i];
                    layers.push(lyr.options.sql);
                  }
                  layers = layers.join(',');
                  uri.source = 'windshaft:' + dsname + '/' + layers;

                  // strip Datasource tags, there's a single one now
                  uri.xml = uri.xml.replace(/\<Datasource\>[\s\S]*?\<\/Datasource\>/g, '');

                  // rename layers to their vector datasource layer
                  // name (how???)
                  // TODO: find a policy, ideally the vector tile
                  //       provider would rename the input layers itself
                  uri.xml = uri.xml.replace(/layer0/g, 'coastline'); 
