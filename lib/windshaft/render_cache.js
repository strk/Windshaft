...
module.exports = function(timeout, mml_store, map_store, mapnik_opts) {
    ...
    me.makeRendererMapnik = function(req, callback){
        ...
        Step(
            ...
            function loadMapnik(err, data){
                ...
                var doing_vector = mapCfg && mapCfg.obj().hasOwnProperty('datasource');
                if ( doing_vector ) {
                  var cfg = mapCfg.obj();
    ...
    function MVTSourceAdaptor(renderer) {
      this.renderer = renderer;
      this.close = function() { /* nothing to do */ };
      this.get = function() { return renderer; };
      this.getTile = this.renderer.getTile.bind(this.renderer);
    ...
    me.makeRendererMVT = function(req, callback){
        var that = this;
        var params = _.extend({}, req.params);
        if ( ! params.token ) {
          callback(new Error("MVT renderer can only be initialized with map token"));
          return;
        }
        if ( params.hasOwnProperty('layer') ) {
          callback(new Error("MVT renderer cannot be initialized with layer number"));
          return;
        }
        ...
        Step(
            ...
            function initRenderer(err, mapConfig) {
              ...
              var dbParams = that.dbParamsFromReqParams(params);
              var format = params.format;
              var MVTSource = dsFactory.getHandler();
              var uri = {
                datasource: {
                  postgres: true
                },
                dbParams: dbParams,
                mapcfg: mapConfig.obj(),
                format: 'pbf'
              }
              // TODO: Use TileliveVector.Backend here instead,
              //       for overzoom etc. features
              new MVTSource(uri, this);
            },
            function makeAdaptor(err, renderer){
                if ( err ) throw err;
                return new MVTSourceAdaptor(renderer);
            },
            function returnCallback(err, renderer){
                callback(err, renderer);
            }
    ...
    me.makeRenderer = function(req, callback){
      ...
      else if ( params.format == 'mvt' ) {
        me.makeRendererMVT(req, callback);
      }
      else {
