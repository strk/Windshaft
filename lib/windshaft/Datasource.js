...
tile_size = 256;
tile_max_geosize = 40075017; // earth circumference in webmercator 3857
...
*/
    ...
    console.log("layer: " + util.inspect(lyr));
...
o.getTile = function(z,x,y,callback) {
  ...
  fetchNext = function(lyrnum) {
    ...
    var sql = that.queries[lyrnum]
      ...
      .replace('{sfx}', sfx)
      .replace('{sfy}', sfy)
      .replace('{bbox}', 'ST_Transform(ST_MakeEnvelope(' + ext.xmin + ','
                                            + ext.ymin + ','
                                            + ext.xmax + ','
                                            + ext.ymax + ','
                                            + '3857), 4326)')
...
o.loadURI = function(cfg, dbParams, uri, callback) {
